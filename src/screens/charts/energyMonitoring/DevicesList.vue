...
const dataList = computed(() => {
  currentPage.value = 1;
  return getCurrentTwins(energyMonitoring.data?.devicesList || {});
});
    ...
    <card-module title="设备列表">
      ...
      <div class="device-list">
        <div class="device-row" v-for="item in currentDeviceList" :key="item.name">
          <div class="device-item device-name" :title="item.name">
            <p class="name">{{ item.name }}</p>
          </div>

          <div class="device-item" v-for="sub in item?.detail" :key="sub.title">
            <div class="device-label">{{ sub.title }}</div>
            <div class="device-value" :class="{ red: sub.value == '关', green: sub.value == '开' }">
              {{ sub.value }}
              <span v-if="sub?.unit">{{ sub.unit }}</span>
            </div>
          </div>

          <div class="device-item device-item-date">
            <div class="device-label">{{ item.time }}</div>
...
.doctor-patient-analysis {
  ...
  .device-row {
    ...
    display: flex;
    align-items: center;
    background: url(@/assets/img/energy/device-list-bg.png) no-repeat;
